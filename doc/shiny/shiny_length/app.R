...
fluidRow(
  column(6,plotOutput("length",height = "200px"))
)
)
...
server <- function(input, output) {
  output$length <- renderPlot({
    selcet.df(input$select_year1,input$select_genre1) %>% ggplot(aes(x=lyrics.length, group=genre, fill=genre)) + geom_density(alpha = 0.2) + xlim(0,800)
