...
fluidRow(
  column(6,plotOutput("freq1",height = "200px"))
),
fluidRow(
  column(6,plotOutput("freq2",height = "200px"))
),
fluidRow(
  column(6,plotOutput("freq3",height = "200px"))
)
)
...
server <- function(input, output) {
  output$freq1 <- renderPlot({
    selcet.df(input$select_year1,input$select_genre1) %>% ggplot(aes(x=affin.count/lyrics.length, group=genre, fill=genre)) + geom_density(alpha = 0.2) + xlim(0,.5)
  }, width = 600, height = 200)
  output$freq2 <- renderPlot({
    selcet.df(input$select_year1,input$select_genre1) %>% ggplot(aes(x=bing.count/lyrics.length, group=genre, fill=genre)) + geom_density(alpha = 0.2) + xlim(0,.5)
  }, width = 600, height = 200)
  output$freq3 <- renderPlot({
    selcet.df(input$select_year1,input$select_genre1) %>% ggplot(aes(x=nrc.count/lyrics.length, group=genre, fill=genre)) + geom_density(alpha = 0.2) + xlim(0,1)
  }, width = 600, height = 200)
}
